...
### 4.3 Holiday campaign folder organisation (all brands — permanent)

**Every Holiday campaign lives inside its brand's `Campaigns/Holiday/` under a folder named for the
event, and each event folder is self-contained.** This keeps Christmas, Father's Day, EOFY, Mother's Day
and every other occasion cleanly separated instead of piling every event's drafts, outputs and reference
images into one shared `Holiday/` folder. Established SS/RDD/SC-2026-HOL-fathers-day.

**Required structure (create it automatically when a new Holiday event begins):**

```
Brands/<BRAND>/Campaigns/Holiday/<Event>/
    Brief/         ← the campaign brief for this event (incl. Design Intent §4.2)
    Draft/         ← working HTML, versioned -draft-vN.html (keep every version, §4.1)
    Output/        ← the single un-versioned final build for preview/QA (§4.1/§9)
    References/     ← reference images + visual inspiration supplied for THIS event only
    Review/        ← QA notes + approval status, referencing the specific Draft version (§8.1)
```

Examples: `Brands/SS/Campaigns/Holiday/Fathers-Day/`, `Brands/SS/Campaigns/Holiday/Christmas/`,
`Brands/RDD/Campaigns/Holiday/Fathers-Day/`, `Brands/SC/Campaigns/Holiday/Fathers-Day/`.

**Rules.**
- **`<Event>` is a stable Title-Case kebab name** for the occasion (`Fathers-Day`, `Christmas`,
  `Mothers-Day`, `EOFY`, `Boxing-Day`, …). One event = one folder, reused for that event every year;
  disambiguate a repeat year in the **filename** (`SS-2026-HOL-fathers-day-…`), not by a new folder.
- **When a new Holiday event is created, automatically scaffold** `Draft/`, `Output/`, `References/`
  (and `Brief/` + `Review/`) inside that event's folder before writing anything.
- **Never mix events.** A reference image, draft or output belongs only in its own event folder — never
  drop Christmas references into the Father's Day folder, and never leave Holiday references loose in an
  unrelated folder.
- **Draft → Review → Output** still governs inside each event folder (§4.1); naming still follows §10
  (`<CODE>-YYYY-HOL-<slug>` for Holiday). The event folder groups the files; it does not change the
  pipeline or the naming convention.
- This organises **Holiday** campaigns specifically. Other cadences/types keep their existing folders
  (`Weekly/`, `Monthly/`, `Product Launch/`, etc.); only `Holiday/` is subdivided by event.

**Lead time — every Holiday campaign must be prepared at least TWO WEEKS before the event (all brands —
permanent; confirmed by Management/Bruce).** Brief, creative, product verification, hosting and the Klaviyo
draft should all be ready **≥ 14 days** ahead of the event date so there is time for review, QA, approval and
a sensible send window. A last-minute Holiday build is a process failure: it forces rushed QA and removes
delivery options (e.g. standard shipping may no longer arrive in time, as happened with Father's Day 2026,
which had to pivot to Local Pickup / Click & Collect messaging where a brand actually offers it). When a
Holiday event is added to the calendar, back-date the brief/creative deadlines to **event minus 14 days** (or
earlier for larger sends per `00-Project Overview/holiday-campaign-framework.md`) and reserve the slot then,
not near the date. This mirrors the "plan ahead, not reactively" principle in
`00-Project Overview/content-calendar.md`.
...
- ✓ footer links (**Unsubscribe · Privacy Policy**, plus **Manage Preferences** only when a brief
  explicitly requests it, §6.26) render as clean text, are clickable, and the layout is unchanged.
...
- Keep it compact: a short contact/help line, the subscription links (**Unsubscribe · Privacy Policy**,
  §6.23 URL-form tags), the company/address line, and an optional copyright line.
- **Do not include a "Manage Preferences" link in generated campaign emails unless explicitly requested.**
  Standard campaign footers across **all brands and all campaign types** retain only the required
  **Unsubscribe** link and the **Privacy Policy** link. Established SS-2026-W38 (Manage Preferences removed
  from the standard SS footer at explicit user request). If a future brief explicitly asks for a Manage
  Preferences / update-details link, add it back using `{% manage_preferences_link %}` per §6.23 — this is
  an opt-in addition, never the default.
...
- **Approved per-brand footer/contact implementation (permanent; established SS-2026-LAUNCH).** Each brand
  ...
  them.** Order at the end of the email: **contact/help block → legal links (Unsubscribe · Privacy Policy,
  §6.23 URL-form tags — Manage Preferences omitted by default, see above) → business name + address.** The
  footer/contact section must sit
...
### 6.29 Equal-height card rows — reserve a fixed-height cell for EVERY wrap-variable region (all brands — permanent; established SC-2026-HOL-fathers-day)

**Any row of cards that carries an icon, a heading and supporting text must stay aligned even when the
heading or the sub-text wraps to a different number of lines between cards.** This is the general rule
behind §6.8 (product cards) and §6.27 (trust cards); §6.29 states it as a reusable convention for **every**
card row (trust/benefit strips, feature rows, "why us" blocks, icon-and-copy rows) and records the exact
verified pattern. Do not wait for a bad render — apply it whenever text length could differ between cards.

**The problem it prevents.** In an HTML email a card sizes to its content. When one card's title fits on one
line and another wraps to two, or one card's sub-text is one line and another is two, the regions **below**
the varying text land at different vertical positions, so the cards' content and their bottoms visually
misalign across the row. Shortening copy is **not** a fix (the next copy change reintroduces it) — the
alignment must be **structural**, exactly as §6.8 requires for product grids.

**The required email-safe pattern (table-based; NOT flex/grid/`min-height` — those are unreliable in
Outlook/Gmail, §6.8/§6.20).**

1. **Equal widths + equal gaps** — the row is one `<table style="table-layout:fixed">` with N equal cells
   (`<td width="(100/N)%">`), each carrying the same side padding as the gap (e.g. `padding:0 5px` → a
   uniform 10px gutter). Fixed layout forces exactly-equal columns regardless of content (§6.27).
2. **A reserved fixed-height `<td>` for the heading region** (`class="tct"`, `height="N"` + `height:Npx`
   in the inline style), sized to the **longest** heading (2 lines by default). A 1-line heading then
   reserves the same box as a 2-line heading, so the region below it starts at the same Y in every card.
3. **A SEPARATE reserved fixed-height `<td>` for the supporting/sub-text region** (`class="tcs"`,
   `height="N"`), sized to the longest sub-text (2 lines by default). Reserving the title alone is **not
   enough** — if the sub-text wraps differently the card bottoms still drift (the exact defect fixed on
   SC-2026-HOL-fathers-day: Round 4 reserved only the title, the subs still wrapped 1 vs 2 lines and the
   bottoms stayed uneven; Round 5 reserving the sub too made all four cards equal height).
4. **Never put `height` and `padding` on the same cell** (§6.20 — content-box vs border-box engines size it
   differently per client). Keep padding on the card's content `<td>`; put the reserved `height` only on the
   bare `.tct`/`.tcs` cells (no padding on them). Put any gap between title and sub in its **own** sized
   spacer row (`<td aria-hidden="true" height="3" style="height:3px;font-size:0;line-height:0;">`), not as
   padding on a height cell.
5. **`height:100%` on the card `<table>`** inside the equal-width cell as a defensive backstop (if content
   ever exceeds a reserved height, the shorter card still stretches rather than leaving a short border).

**Verified reference structure** (the "We've got you covered" strip; one card, four identical siblings):
```
<td class="tc" width="25%" valign="top" align="center" style="padding:0 5px;">                     <!-- equal width + 10px gap -->
  <table role="presentation" width="100%" ... style="...border-radius:10px;height:100%;">           <!-- card, height:100% backstop -->
    <tr><td align="center" valign="top" style="...padding:18px 8px 16px;">                          <!-- content cell: padding, NO height -->
      <table ... align="center" style="margin:0 auto 10px;"><tr><td width="42" height="42" ...>ICON</td></tr></table>
      <table role="presentation" width="100%" ...>
        <tr><td class="tct" align="center" valign="top" height="34" style="height:34px;">HEADING</td></tr>   <!-- reserved 2 lines; height, NO padding -->
        <tr><td aria-hidden="true" height="3" style="height:3px;font-size:0;line-height:0;mso-line-height-rule:exactly;">&nbsp;</td></tr>
        <tr><td class="tcs" align="center" valign="top" height="30" style="height:30px;">SUB-TEXT</td></tr>  <!-- reserved 2 lines; height, NO padding -->
      </table>
    </td></tr>
  </table>
</td>
...
**Desktop vs mobile.** On desktop the reserved `.tct`/`.tcs` heights keep every card aligned even when text
wraps differently. On mobile the cards stack full-width, so the reserved heights **must reset to auto** —
add the classes to the media-query height reset next to the product-card classes:
`@media (max-width:600px){ .pnc,.pdc,.ppc,.tct,.tcs{height:auto !important} }` (and `.tc{display:block;width:100%}`
to stack). Stacked cards then size naturally and stay responsive.

**Future implementation rule.** For **any** future email card row with icon + heading + supporting text,
proactively build it with the reserved-height `.tct` (heading) and `.tcs` (sub-text) cells from the start —
whenever wrapping could differ between cards. A card row must never need manual re-alignment after rendering
just because one card has more wrapped lines than another. Reuse the class names `.tct`/`.tcs` (heading /
sub-text reserved cells) so the pattern is recognisable across templates. Combine with §6.8 (product grids),
§6.20 (no height+padding on one cell) and §6.27 (trust row reuses the grid container).
...
### 13.5 Campaign → Klaviyo Synchronization Rules (permanent, all brands)

Permanent rule for keeping a Klaviyo Campaign **Draft** in sync with its approved Campaign HTML. It
mirrors the **Klaviyo Flow and Claude Code** reference pattern (*deterministic name → upsert → verify*,
`publish-flow-template.js`) and **reuses the existing Campaign services** — it adds no new send/schedule
capability (§8.1/§9/§13 stand unchanged).

- **The Campaign `Output/` HTML is the source content.** When an approved Campaign HTML changes — any
  legitimate edit (content, copy, products, product links/images, prices, CTA, layout, design,
  header/footer, branding, spacing/style) — its corresponding Klaviyo **Template** and its existing
  Klaviyo **Campaign Draft** must be re-synchronised so the Draft reflects the latest HTML. Never
  hand-paste HTML into Klaviyo.
- **Command:** `node platform/integrations/klaviyo/sync-campaign-draft.js --brand <CODE> --campaign
  <campaign_id> [--file <output-path>] [--dry-run]` (npm: `sync:campaign`). HTML source = the campaign's
  `config/approved-html.json` entry, or `--file`.
- **Identity is deterministic, by NAME — no IDs are stored.** Template = `Automation: <campaign_id>`
  (upsert by exact name → **update** if it exists, else create; the source template's identity is stable).
  Draft = the campaign whose name contains `<campaign_id>` as a whole boundary token (`RDD-2026-3` never
  matches `RDD-2026-37`).
- **UPDATE existing, never duplicate.** Always find-existing → **verify identity** → update. Never create
  a second Template or Draft for the same campaign, and never overwrite an unrelated campaign.
- **Brand isolation is mandatory (§12).** The campaign_id prefix must match `--brand`, and each brand
  syncs through its OWN Klaviyo account/key (RDD `KLAVIYO_API_KEY`, SS `SS_KLAVIYO_API_KEY`, SC
  `SC_KLAVIYO_API_KEY`). A cross-brand id is refused before any API call.
- **Only `Draft` campaigns may be synced.** The command STOPS and writes nothing if: **no** matching
  campaign exists (it will **not** create one), the matched campaign is **not** a Draft
  (Sent/Scheduled/Sending/…), or **more than one** Draft matches (ambiguous — it never guesses).
  Automatically creating a new Draft is out of scope and requires a separately-approved opt-in (with the
  §13.1 audience-confirmation gate).
- **Never** send, schedule, publish, activate, change audience/recipients, change sender, or change
  timing. The sync touches only the Template and the Draft message's HTML + (optionally) subject/preview
  — never audience, `send_strategy`, or a send-job (the `safety.js` guard scans the file).
- **Verify after every sync (a 2xx is never trusted):** re-fetch and confirm status is still `Draft`, the
  Draft message carries the new HTML, exactly one Draft matches (no duplicate), and no `send_strategy` is
  present; then report success/failure.
- **Applies to every campaign** this project supports (SS, SC, RDD, and future brands).

**Recommended future trigger (documented; NOT yet installed).** Keep the sync a manual, QA-gated step for
now:

```
Campaign HTML change → QA → sync:campaign → Klaviyo Template update → existing Draft update → verify
```

A later automatic trigger MAY use a Claude Code **PostToolUse** hook on edits/writes to `**/Output/*.html`,
but only after the command is proven stable, and it must preserve every guard above (Draft-only, no
duplicate, no send/schedule, brand isolation, verify-after-write) and run only when the campaign_id can be
safely resolved from the changed file.
...
_Updated: 2026-08-18 · §13.5 Campaign → Klaviyo Synchronization Rules — new `sync-campaign-draft.js` (npm `sync:campaign`): approved Output HTML → Template upsert → EXISTING Draft update; Draft-only, no duplicate, no send/schedule, brand isolation, verify-after-write; STOPS on missing/non-Draft/multiple. Future PostToolUse trigger documented, not installed. Additive; reuses existing services; extends §13._
